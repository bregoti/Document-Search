				...
				<c:otherwise>
					...
					<c:forEach var="doc" items="${page.content}" varStatus="status">
						<div class="ui-widget-content ui-corner-all" style="margin-top: .5em; padding-left: .5em; background: ${status.index %2==0 ?'#eee':'#fff'}">
									<p><h3><a href="<c:url value="/downloadFile/?filePath=${doc.resourcename}" />"  target="_blank" >${doc.fileName}</a></h3></p>
									<c:if test="${not empty doc.title}">
										<p><span class="text-success"><strong>Title:</strong></span>  <c:out value="${doc.title}"/></p>
									</c:if>
									<c:if test="${not empty doc.subject}">
										<p><span class="text-success"><strong>Subject:</strong></span>  <c:out value="${doc.subject}"/></p>
									</c:if>
						        	<c:if test="${not empty doc.keywords}">
										<p><span class="text-success"><strong>Keywords:</strong></span>  <c:out value="${doc.keywords}"/></p>
									</c:if>
						        	<c:if test="${not empty doc.author}">
										<p><span class="text-success"><strong>Author:</strong></span>  <c:out value="${doc.author}"/></p>
									</c:if>
						        	<c:if test="${not empty doc.resourcename}">
										<p><span class="text-success"><strong>File Location:</strong></span>  <c:out value="${doc.resourcename}"/></p>
									</c:if>
						        	<c:if test="${not empty doc.lastModified}">
										<p><span class="text-success"><strong>Last Modified:</strong></span>  <c:out value="${doc.lastModified}"/></p>
									</c:if>
								<p>
